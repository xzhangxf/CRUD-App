...
class VisitFormViewController: UITableViewController {
    ...
    private lazy var modeControl : UISegmentedControl = {
        var items = Visit.TransportMode.allCases.map { $0.description }
    ...
    @objc private func saveTapped() {
        ...
        if let existing = visit{
            ...
            //delegate?.didAddVisit(update)
            delegate?.didUpdateVisit(update)
        }   else {
            
            let created = Visit(title: title, mode: mode, date: date, distanceMeters: distance, note: note)
            delegate?.didAddVisit(created)
        }
//        if var v = visit {
//            // update
//            v.title = title
